...
def training_loop(
    ...
    for epoch in range(1, epochs + 1):
        ...
        # Training
        running_loss, running_total_seq, running_correct_preds, running_total_tokens = (
            0.0,
            0,
            0,
            0,
        ...
        for i, batch in enumerate(train_loader):
            ...
            running_correct_preds += torch.sum(
                torch.argmax(logits, dim=-1)[mask_pad] == target_ids[mask_pad]
            ).item()
            ...
            if (
                i > 250
            ):  # stop the epoch after 250 batches (simply to log more often when using lots of data)
                break
        ...
        val_loss, val_acc = evaluate(
            model, valid_loader, criterion, device, pad_idx, vocab_size
        )
